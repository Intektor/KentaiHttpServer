...
class CommandSendNotification : Command {
    ...
    override fun execute(name: String, args: List<String>) {
        ...
        DatabaseConnection.ds.connection.use { connection ->
            connection.prepareStatement("SELECT fcm_token FROM kentai.login_table WHERE username = ? OR user_uuid = ?").use {
                it.setString(1, args[0])
                it.setString(2, args[0])
                val query = it.executeQuery()
                if (!query.next()) {
                    KentaiServer.logger.info("No user found with uuid or username: ${args[0]}!")
                    return
                } else {
    ...
    override fun logHelp() {
        KentaiServer.logger.info("userUUID/username message")
    }
