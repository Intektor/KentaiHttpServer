...
import de.intektor.mercury_server.LastOnlineType
import de.intektor.mercury_server.hasProfilePictureBeenUpdated
    ...
    override fun handlePacket(packet: UserPreferencePacketToServer, socketFrom: Socket) {
        ...
        if (todoList.isNotEmpty()) {
            DatabaseConnection.ds.connection.use { connection ->
                ...
                connection.prepareStatement("SELECT last_time_online, type_closed, user_uuid FROM mercury.user_status_table WHERE user_uuid IN ($inClause)").executeQuery().use { query ->
                    while (query.next()) {
