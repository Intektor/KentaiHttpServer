        ...
        if (packet.interested) {
            ...
            if (DirectConnector.clientMap.containsKey(userUUID)) {
                ...
                DatabaseConnection.ds.connection.use { connection ->
                    connection.prepareStatement("SELECT last_time_online, type_closed, user_uuid FROM mercury.user_status_table WHERE user_uuid = '$userUUID'").executeQuery().use { query ->
                        while (query.next()) {
