...
class UploadReferenceHandler : AbstractHandler() {
    ...
    override fun handle(target: String, baseRequest: Request, request: HttpServletRequest, response: HttpServletResponse) {
        ...
        DatabaseConnection.ds.connection.use { connection ->
            connection.prepareStatement("SELECT state, times_tried FROM mercury.references WHERE reference_uuid = ?").use { statement ->
                ...
                statement.executeQuery().use { query ->
                    if (query.next()) {
                        ...
                        connection.prepareStatement("INSERT INTO mercury.references (reference_uuid, state, times_tried, upload_time) VALUES(?, ?, 1, ?)").use { statement2 ->
    ...
    private fun receiveData(inputStream: InputStream, timesTried: Int, connection: Connection, referenceUUID: String) {
        try {
            ...
            connection.prepareStatement("UPDATE mercury.references SET state = ? WHERE reference_uuid = ?").use { statement ->
                statement.setInt(1, State.FINISHED.ordinal)
                statement.setString(2, referenceUUID)
                statement.execute()
            }
        } catch (t: Throwable) {
            connection.prepareStatement("UPDATE mercury.references SET times_tried = times_tried + 1 WHERE reference_uuid = ?").use { statement ->
                statement.setString(1, referenceUUID)
                statement.execute()
            }
            if (timesTried > 10) {
