...
class UsersRequestHandler : AbstractHandler() {
    ...
    override fun handle(target: String, baseRequest: Request, request: HttpServletRequest, response: HttpServletResponse) {
        ...
        DatabaseConnection.ds.connection.use { connection ->
            connection.prepareStatement("SELECT user_uuid, username, message_key FROM mercury.login_table WHERE user_uuid IN(${"?, ".repeat(r.users.size).dropLast(2)})").use { statement1 ->
