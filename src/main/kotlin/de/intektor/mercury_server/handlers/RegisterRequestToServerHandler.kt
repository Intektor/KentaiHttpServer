    ...
    override fun handle(target: String, baseRequest: Request, request: HttpServletRequest, response: HttpServletResponse) {
        ...
        if (req.username.length !in 5..20 || !req.username.matches(("\\w+".toRegex()))) {
            ...
            DatabaseConnection.ds.connection.use { connection ->
                connection.prepareStatement("SELECT username FROM mercury.login_table WHERE username = ?").use { statement ->
                    statement.setString(1, req.username)
                    if (!statement.executeQuery().next()) {
                        val userUUID = UUID.randomUUID()

                        connection.prepareStatement("INSERT INTO mercury.login_table (username, user_uuid, message_key, auth_key, fcm_token) VALUES (?, ?, ?, ?, ?)").use { statement2 ->
