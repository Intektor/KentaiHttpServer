...
import de.intektor.mercury_server.tcp.handlers.*
...
object DirectConnector {
    ...
    fun unregisterClient(userUUID: UUID, properlyClosed: Boolean) {
        ...
        DatabaseConnection.ds.connection.use { connection ->
            connection.prepareStatement("DELETE FROM mercury.user_status_table WHERE user_uuid = ?").use { statement ->
                statement.setString(1, userUUID.toString())
                statement.execute()
            }

            connection.prepareStatement("INSERT INTO mercury.user_status_table (user_uuid, last_time_online, type_closed) VALUES (?, ?, ?)").use { statement ->
                statement.setString(1, userUUID.toString())
    ...
    class ConnectionThread(private val clientSocket: Socket, private val dataIn: DataInputStream) : Thread() {
        ...
        override fun run() {
            ...
            try {
                while (keepConnection) {
                    val packet = readPacket(dataIn, MercuryTCPOperator.packetRegistry, Side.SERVER)
